...
class TkPanel(Panel):
  ...
  def init_lights(self):
    ...
    if self.orientation == 'H':
      ...
      self.lights[2] = LocatedLight(2, (row, col + 2*add))
      self.lights[3] = LocatedLight(3, (row, col + 3*add))
    if self.orientation == 'V':
      self.lights[0] = LocatedLight(0, (row, col))
      self.lights[1] = LocatedLight(1, (row + add, col))
      self.lights[2] = LocatedLight(2, (row + 2*add, col))
      self.lights[3] = LocatedLight(3, (row + 3*add, col))
